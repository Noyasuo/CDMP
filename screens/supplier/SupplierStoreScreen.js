  ...
  const fetchProducts = async () => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Filter products that belong to the current user with robust matching
            const myProducts = data.filter(item => {
                ...
                // Robust comparison: check existence, convert to string, trim, lowercase
                if (!ownerName || !username) return false;
            ...
            console.log(`[SupplierStore] Found ${myProducts.length} products owned by ${username}`);
